...
This historical stage introduced a per-OS-user HTTP gate with persisted quota observations, conservative accounting, and clock continuity checks. It has since been removed: `wcl_raid_coach/api.py` now reports HTTP 429 directly, without preflight quota probes or local scheduling state. Report/cache locks remain owned by `dataset.py` for Complete Bundle reuse and resumable collection.
