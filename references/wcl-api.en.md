...
`coach triage` uses one WclClient, one full report metadata query, and its in-memory token. It checks Report Revision after Mechanic Review, before each Focused Evidence Window, and again after that window's pagination. Windows retain targetID filtering, fixed ranges, explicit null termination, and local participant filtering. API failure or a revision change rejects the combined result without persisting event evidence.

The client retries transient connection failures and HTTP 500, 502, 503, and 504 responses with exponential backoff. HTTP 429 immediately returns `wcl_rate_limit` for the caller to report to the user. It does not retry 429, persist cooldown, or block subsequent independent requests.

Data requests call the API directly without quota probes, reservations, or estimated debits. Response `rateLimitData` is used only for output and diagnostics; `doctor` can still explicitly query quota to check connectivity. There is no shared HTTP scheduling lock or persisted clock baseline. Legacy `~/.wcl-report-data/api/` state is ignored.

Report/cache locks still coordinate downloads within the same data directory. Repeated `prepare` validates and reuses Complete Bundles for the same Report Revision. Interrupted or rate-limited collection retains Raw Pages and checkpoints and resumes from the unfinished page. Report metadata is still queried to detect Report Revision changes. Mechanic Review and Focused Evidence Window events are transient, so repeated calls still collect them.
