...
The canonical Personal Review workflow starts after Boss Attempt/player selection and before target Complete Bundle retrieval; elapsed includes gaps between later CLI invocations. WCL network time remains marked `not_measured`. Because the CLI cannot directly observe out-of-process Agent synthesis, that stage is `unavailable` and final `target_met` is `null`.
