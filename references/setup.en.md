...
## Requests and data reuse

HTTP 429 returns `wcl_rate_limit` immediately without retries or persisted cooldown. Legacy `~/.wcl-report-data/api/state.json` and `schedule.lock` are ignored, so restarts or system clock changes cannot block requests through those files. File cleanup or a `doctor` unlock is unnecessary. `doctor` still checks credentials, storage, and API connectivity.

Report/cache locks continue coordinating `prepare` within the same data directory: validated Complete Bundles for the same Report Revision are reused, and incomplete downloads resume from Raw Page checkpoints. Report metadata is still queried to detect Report Revision changes. Separate data/cache roots do not share data deduplication. See [API behavior](wcl-api.en.md).
