...
## Mixed-report zone resolution

For a Retail report whose principal zone is not a raid but which contains non-Mythic+ Encounters, the client uses official `worldData.zones` to find exactly one raid zone containing all those Encounters. The resolved zone becomes `report.zone`; the original is preserved as `report.principal_zone`. Missing or ambiguous matches are rejected; difficulty tables from multiple zones are never merged. Ordinary raid and pure Mythic+ reports do not trigger this supplemental query.
