...
Immediately after Boss Attempt and player selection, and before target Complete Bundle retrieval, a canonical blocked workflow must be created with the not-yet-existing Personal Analysis path. Later calls inherit the same origin through `--previous-workflow`, conservatively including wall time between invocations. All three to nine qualified Reference Samples must remain in the workflow and Benchmark. The CLI derives retrieval, Agent synthesis, validation, and rendering `stage_progress` from known artifacts and accepts no caller timing. A delivery artifact is sampled only through HTML/index verification; finalization resamples after the atomic delivery write before claiming persistence. Because the CLI cannot directly observe Agent synthesis duration, that stage is `unavailable` and final `target_met` must be `null`; total elapsed alone cannot prove the 180/30-second target.
