...
def _render_mechanic_html(document: dict[str, Any]) -> str:
    ...
    source_links = " · ".join(
        f'<a href="{escape(source, quote=True)}" rel="noreferrer">{escape(source)}</a>'
        for source in document["ruleset"]["sources"]
    )
...
<div class="report">
  ...
  <div class="shell">
    ...
    <footer><div><p>{escape(document['scope_note'])}</p><p>{escape(labels['sources'])}: {source_links}</p></div><code>{escape(identity['report_code'])} / Revision {identity['report_revision']} / {escape(document['ruleset']['version'])} / {escape(document['document_id'][:12])}</code></footer>
  </div>
</div>
</body>
</html>
...
"""
_CSS += ".report{overflow-wrap:anywhere}#theme-auto:focus-visible~.report label[for=theme-auto],#theme-light:focus-visible~.report label[for=theme-light],#theme-dark:focus-visible~.report label[for=theme-dark]{outline:3px solid var(--accent);outline-offset:2px}"
